...
| 技术类别     | 关键技术                     | 优化目标   | 相关文档                                 |
| -------- | ------------------------ | ------ | ------------------------------------ |
| **计算优化** | FlashAttention, 算子融合 | 延迟 ↓ | [[推理 引擎 原则#计算图优化]] |
| **内存优化** | PagedAttention, KV Cache | 吞吐量 ↑  | [[推理 引擎 原则#内存管理]] |
| **模型压缩** | 量化, 剪枝, 蒸馏               | 资源消耗 ↓ | [[模型 压缩 蒸馏]]   |
| **系统优化** | 批处理, 并行, 调度              | 并发能力 ↑ | [[推理 引擎 调优#系统级优化]]    |

### 🛠️ 实践应用
| 应用场景       | 推荐引擎                | 关键考虑      | 相关文档                                 |
| ---------- | ------------------- | --------- | ------------------------------------ |
| **高并发API** | vLLM, TGI | 吞吐量, 延迟 | [[推理 引擎 选择#场景匹配指南]] |
| **边缘部署** | llama.cpp, ONNX | 资源限制, 移植性 | [[推理 引擎 选择#场景匹配指南]] |
| **实时交互** | TensorRT, Triton | 延迟, 准确性 | [[推理 引擎 选择#场景匹配指南]] |
| **批量处理** | DeepSpeed, Megatron | 吞吐量, 成本 | [[推理 引擎 选择#场景匹配指南]] |
...
**学习内容**：
- [[推理 引擎 原则#学习目标]] - 核心术语和原理
- [[推理 引擎 选择#选型目标与原则]] - 从llama.cpp开始
- [[推理 引擎 调优#调优目标与策略]] - 量化和批处理基础
...
**学习内容**：
- [[推理 引擎 原则#内存管理]] - KV Cache, PagedAttention
- [[推理 引擎 原则#计算图优化]] - FlashAttention, 算子融合
- [[推理 引擎 调优#性能分析方法论]] - 性能分析工具使用
...
**学习内容**：
- [[推理 引擎 调优#系统级优化]] - 并行、批处理、调度
- [[推理 引擎 选择#选型决策流程]] - 引擎选型和架构设计
- [[推理 引擎 监控#监控最佳实践]] - 性能监控和告警
...
| 术语 | 定义 | 相关文档 |
|------|------|----------|
| **吞吐量 (Throughput)** | 单位时间内处理的请求数或token数 | [[推理 引擎 原则#性能分析与监控]] |
| **延迟 (Latency)** | 单个请求从输入到输出的时间 | [[推理 引擎 原则#性能分析与监控]] |
| **KV Cache** | 键值缓存，存储历史token的K/V向量 | [[推理 引擎 原则#内存管理]] |
| **PagedAttention** | 分页注意力机制，类似OS虚拟内存 | [[推理 引擎 原则#内存管理]] |
| **连续批处理** | 动态管理请求进出的批处理技术 | [[推理 引擎 调优#连续批处理优化]] |
| **量化 (Quantization)** | 降低模型权重精度以减少资源消耗 | [[模型 压缩 蒸馏#量化方法]] |
...
### 相关知识领域
- [[LLM 训练 流水线]] - 训练流程与推理优化的关系
- [[Transformer 架构]] - Transformer架构对推理的影响
