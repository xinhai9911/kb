...
created: 2026-07-29
summary: >-
    针对 NPP 流表老化清理的性能测试实例：测试目标、观测口径（show cpu / show runtime / show errors）、四类用例（基线转发、空闲清理开销、满表压力、多核扩展）、具体 CLI 与采样步骤，以及示例结果表与判定标准。配套 [[50-reference/npp-flowtable-cleanup-example|清理代码实例]] 与 [[50-reference/npp-timer-mechanism|定时触发机制]]。
category: reference
updated: 2026-07-29
sources: []
base_confidence: 0.8
...
> 配套：[[50-reference/npp-timer-mechanism|NPP 内部定时触发机制]]（架构与超时）、[[50-reference/npp-flowtable-cleanup-example|NPP 流表清理代码实例]]（被测代码）、[[50-reference/vpp-plugin-perf|VPP 插件性能调优]]（调优方法论）。
...
> 若插件未暴露 `show flowtable sessions`，可在 `flowtable_main.per_thread_data[ti].session_count` 上补一个 CLI（见 [[50-reference/vpp-plugin-dev|插件开发]] §CLI）。
...
**判定**：`show runtime` 中每个 worker 的 `flowtable-cleaner` `calls` 大致相等（流亲和良好、无跨核争锁）；总清理吞吐随 worker 数近似线性。若某 worker 明显偏高 → 检查 RSS 是否把多流打到同一核（见 [[50-reference/vpp-plugin-perf|调优]] §流绑定）。
...
2. **per_call 耗时高** → 见 [[50-reference/vpp-plugin-perf|调优]]：slow_cleanup 里是否有 per-session 分配、LRU 操作是否缓存友好。
3. **满表不回收 / 内存涨** → 检查 `fastcleanum` 触发条件、`pool_put` 是否配对 `pool_get`、del_handler 是否抛错中断清理。
4. **多核不均** → 检查 RSS / 流哈希、NUMA 亲和（[[50-reference/vpp-plugin-perf|调优]] §多核）。
...
- 清理实现：[[50-reference/npp-flowtable-cleanup-example|NPP 流表清理代码实例]]
- 触发机制：[[50-reference/npp-timer-mechanism|NPP 内部定时触发机制]]
- 调优方法：[[50-reference/vpp-plugin-perf|VPP 插件性能调优]]
