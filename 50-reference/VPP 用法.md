...
created: 2026-07-29
summary: >-
    VPP 的日常使用速查：启动方式、startup.conf 关键配置、CLI 常用命令（接口/路由/邻居/插件）、抓包与调试、常见运维操作。与 [[20-protocols/VPP 2|VPP 知识]] 配合阅读。
category: reference
updated: 2026-07-29
sources: []
base_confidence: 0.8
...
> 知识背景见 [[20-protocols/VPP 2|VPP 知识]]。本文是"怎么用"的操作手册。
...
- 本项目 [[50-reference/NPP 定时器 机制|NPP]] 在 VPP 节点图里挂了 process node 做流表老化，调试时可用 `show node`、`show runtime` 看该节点调用情况。
- 写自定义插件时，参考 `VLIB_REGISTER_NODE` 宏与 `vlib_process_*` 系列 API（见 [[20-protocols/VPP 2|VPP 知识]] §核心概念）。
