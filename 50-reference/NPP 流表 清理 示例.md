---
aliases: ["npp-flowtable-cleanup-example"]
title: NPP 流表清理代码实例
tags: [npp, vpp, flowtable, snippet, reference, active]
created: 2026-07-29
summary: >-
    基于 [[50-reference/NPP 定时器 机制|NPP 定时触发机制]] 的流表老化清理完整代码实例：三级协作（clear-process 定时调度 → cleaner 中断执行 → session_try_cleanup 策略）的可编译骨架，含 per-thread 无锁清理、slow/fast 双策略与 exapi del_handler 解耦回调。
category: reference
updated: 2026-07-29
sources: []
base_confidence: 0.8
...
> 配套说明见 [[50-reference/NPP 定时器 机制|NPP 内部定时触发机制]]（架构、空闲判定、超时表）。
> 性能取舍见 [[50-reference/VPP 插件 性能|VPP 插件性能调优]]。
...
- 架构与超时表：[[50-reference/NPP 定时器 机制|NPP 内部定时触发机制]]
- 性能取舍：[[50-reference/VPP 插件 性能|VPP 插件性能调优]]
- 插件写法：[[50-reference/VPP 插件 开发|VPP 插件开发]]
