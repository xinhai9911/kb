...
created: 2026-07-29
summary: >-
    VPP 自定义插件的开发方法：插件目录结构、VLIB_REGISTER_NODE 注册节点、node function 编写范式、Process Node 协程、VNET 功能挂接、CMake 构建、加载与调试。结合 [[50-reference/npp-timer-mechanism|NPP 定时触发机制]] 的实际写法。
category: reference
updated: 2026-07-29
sources: []
base_confidence: 0.8
...
> 前置：先读 [[20-protocols/vpp|VPP 知识]]（节点图、Process Node、缓冲管理）与 [[50-reference/vpp-usage|VPP 使用方法]]（CLI、插件加载）。本文聚焦"如何写一个 `.so` 插件"。
...
- 真实例子见 [[50-reference/npp-timer-mechanism|NPP]]：`flowtable_cleaner_node` 是一个 INTERNAL node，由 process node 触发。
...
本项目 [[50-reference/npp-timer-mechanism|NPP]] 是 VPP 插件范式的真实样例：
...
- [[20-protocols/vpp|VPP 知识]]、[[50-reference/vpp-usage|VPP 使用方法]]
