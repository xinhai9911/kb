---
aliases: ["intel-architecture-perf"]
...
- **向量化**：SSE/AVX 指令集与优化。
- **性能调优**：与 [[50-reference/NPP 定时器 机制]]、[[50-reference/dlopen internal 内存]] 中的底层机制互补。
...
- 底层机制笔记：[[50-reference/NPP 定时器 机制]]、[[50-reference/dlopen internal 内存]]
...
- 底层机制（定时器、动态链接）与 B 卷执行环境、控制寄存器呼应 → [[50-reference/NPP 定时器 机制]]、[[50-reference/dlopen internal 内存]]。
