...
> 结构来自 **全量 OCR（436 页，2026-07-29）** 的目录与正文，章名/节名已核对，非推断。

- **定位**：**入门→中级实战书**（作者自序：2012 年起用 ES，从 0.19 到 2.3，书中针对当时中文资料滞后的痛点，覆盖 HTTP JSON 接口与 Java 接口双路线）。强调"先基础、后由浅入深：索引→映射→搜索→聚合→分词→集群→ELK 实战"。
- **全书 10 章，结构（全量 OCR 核实）**：
  - **第1章 Elasticsearch 入门**：ES 是什么（基于 Lucene 的分布式 RESTful 搜索引擎、文档数据库）、历史与相关产品（**Beats / Shield / Watcher / Marvel**）、全文搜索与 **Lucene 倒排索引**（词典/频率/位置文件、<前缀,后缀> 压缩、数字差值压缩）、术语概念（term/index/type/document 等）、JSON、**安装配置（Java/ES/配置/运行/服务/升级）**、对外接口（**REST 约定、Head 插件、CRUD 操作**）、**Java 接口**（1.6 节：创建/增加/修改/查询/删除文档）。
  - **第2章 索引**：索引管理（2.1 创建/删除/获取/打开关闭索引）、索引映射管理（2.2）、索引别名（2.3）、索引配置（2.4 更新/获取/分析/重建索引）、索引监控（2.5 统计/分片/分片存储）、状态管理（2.6 刷新/冲洗/合并）、**文档管理（2.7 增/删/查/多文档/更新接口）**。
  - **第3章 映射**：概念（3.1）、**字段数据类型**（3.2 核心/地理/专门类型）、**元字段 meta**（3.3 `_all`/`_id`/`_index`/`_routing`/`_source`/`_type`/`_uid` 等）、**映射参数**（3.4 analyzer/boost/coerce/copy_to/doc_values/dynamic/fielddata/format/ignore_above/index_options/null_value/search_analyzer/store 等二十余项）、动态映射（3.5 概念/字段映射/动态模板/重写默认模板）。
  - **第4章 搜索**：搜索方式（4.1 重新评分 rescore/滚动 scroll/隐藏内容查询/搜索函数）、**查询 DSL（4.2 查询与过滤区别、字段/复合/连接/地理/跨度查询、高亮）**、简化查询。
  - **第5章 聚合**：**聚合分类**、**度量聚合（5.2 平均/基数/最大/最小/和/值计数/统计/百分比/脚本度量/地理边界/地理重心）**、**分组聚合（5.3 子聚合/直方图/日期直方图/范围/过滤/多重过滤/空值/嵌套/采样/重要索引词/索引词/地理点距离/地理哈希网格/IPv4 范围）**、**管道聚合（5.4 平均/移动平均/总和/最大/最小/百分位/差值/串行差分/分组选择器）**。
  - **第6章 集群管理**：含**主节点选举（6.4.1）、故障检测（6.4.2）、集群平衡**等分布式运维主题。
  - **第7章 索引分词器**：分词器（analyzer）配置与使用。
  - **第8章 高级配置**：进阶参数与调优配置。
  - **第9章 告警、监控和权限管理**：对应 **Shield（权限/安全）、Watcher（告警）、Marvel（监控）** 三大 X-Pack 前身组件的管理。
  - **第10章 ELK 应用**：**ELK 集成实战**，含 **Nginx 日志**范例——用 Logstash 2.3.2 写 `logstash_nxlog.conf` 采集 Nginx 访问日志进 ES 做分析。
- **可提炼要点（OCR 实证）**：
  - **Lucene 倒排索引压缩**：词典关键词压缩为 `<前缀长度, 后缀>`（如前词"阿拉佾"→"阿拉佾语" 存 `<3,语>`）；文档号存差值以省字节。
  - **ES 周边产品**：Beats（Filebeat/Topbeat/Packetbeat 采集）、Shield（安全，收费）、Watcher（告警，收费）、Marvel（监控，收费）——书中标注均为收费组件。
  - **双接口路线**：HTTP JSON 与 Java API 可互转（HTTP 最终转 Java），作者团队选 Java 接口（效率高、少端口、易升级鉴权）。
  - **聚合体系完整**：度量/分组/管道三大类，含地理聚合、管道聚合（移动平均、串行差分），与 [[20-protocols/elasticsearch]] 聚合层互补。
- **关联**：ELK 链路与 [[sources/chips/h3c-tap]]（流量可视化）、[[20-protocols/elasticsearch]] 协议层互补；Nginx 日志范例可与 [[sources/books/network-hcna-hcnp]] 的运维场景对照；分词器/映射思路可用于日志分析字段设计。
