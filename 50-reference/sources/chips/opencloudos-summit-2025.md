---
title: OpenCloudOS 2025 生态大会资料蒸馏
tags: [reference, sources, opencloudos, linux, summit, observability, active]
created: 2026-07-29
updated: 2026-07-29
source_dir: Q:\芯片资料\2025OpenCloudOS操作系统生态大会等1个文件
---

# OpenCloudOS 2025 生态大会资料蒸馏

> 2025 OpenCloudOS 操作系统生态大会 PPT 合集（含 zip 原包），分三大论坛。

## 来源

- 压缩包：`Q:\芯片资料\2025OpenCloudOS操作系统生态大会等1个文件.zip`
- 解压目录：`Q:\芯片资料\2025OpenCloudOS操作系统生态大会等1个文件\`

## 论坛与主题

### 技术创新论坛
- 刘泽佑 — AI 多智能体协同：Linux 发行版软件包无人值守维护工作流再造
- 周昱杉 — 云上网络可观测性的新革命：颠覆传统的全景洞察
- 宋恺睿 — Swap Table 架构革新
- 李伟杰 — OCAI：开放智能体驱动的智能化运维和分析新范式
- 李辉 — 云原生可观测性增强与隔离实践
- 杨达明 — AI Agent 实时驱动：Linux 内核漏洞补丁移植自动化
- 段永超 — 利用 perf-prof 进行系统问题分析

### 最佳实践论坛
- 冯立亮 — 嘉为科技与 OpenCloudOS 生态共建
- 叶伟 — 沐曦 GPU 训练及推理优化实践
- 张同浩 — HUATUO 操作系统深度可观测驱动的故障根因定位
- 杨金泽 — 宝塔面板轻量云运维实践
- 程铖 — 绿盟协同安全体系下的漏洞精准探测
- 郭一兵 — 达梦与 OpenCloudOS 软件栈联合优化

### 生态协同论坛
- 孙敏 — RVA23 路线下的 RISC-V 架构创新与生态协同
- 高杰欣 — 扎根 OC 生态的年度协同实践

## 关键要点

- **可观测性**：perf-prof、云原生可观测、深度可观测根因定位（与 [[50-reference/npp-timer-mechanism]] 同属可观测主题）。
- **AI 运维**：多智能体、AI Agent 驱动补丁/运维。
- **异构**：沐曦 GPU、RISC-V（RVA23）、达梦数据库优化。

## 适用场景

- 系统可观测性 / 智能化运维方案调研时回查演讲。
- 跟踪国产 OS 与异构算力生态。

## 关联

- 可观测/定时器机制：[[50-reference/npp-timer-mechanism]]、[[50-reference/dlopen-internal-memory]]

---

## 深度提炼

> 来源：2025 OpenCloudOS 生态大会 PPT 合集（`Q:\AI\extract_tmp\out\` 下多份 txt，含 perf-prof、HUATUO、OCAI、沐曦 GPU、RISC-V、达梦、宝塔等）。中文正文部分为乱码但**英文标题/术语/框架名完整可辨**，以下提炼均来自可识别原文结构。三大论坛：技术创新 / 最佳实践 / 生态协同。

### 一、系统级性能分析与可观测（核心主题）

**① perf-prof — 系统级性能分析工具（段永超，腾讯云）**
- 定位：**安全可靠·长期运行·广泛兼容·零存储开销**的系统级性能分析工具。
- 架构：**内核态（事件源 + 过滤器）→ 用户态（分析器）**；核心是事件处理管道 + 多个分析单元。
- 分析单元（`分析单元–` 系列页）：`task-state`、SQL 聚合分析、`top` 聚合分析、延迟分析、`multi-trace`（多踪联合）。特点：**一条命令**完成采集+分析。
- 场景：虚拟化场景联合分析；探索 **AI 辅助**分析。

**② HUATUO（华佗）— 操作系统深度可观测故障分析（张同浩，CCF HUATUO 开源社区）**
- 背景：系统故障分析四大挑战——缺乏故障现场（漂移止损/预案降级）、复现难（偶发毛刺）、人力成本高、缺乏持续全景观测（现有指标只能表征状态，无法根因定位）。
- 核心特性：✓ 低损耗内核全景观测 ✓ 异常事件驱动诊断 ✓ 全自动化追踪 AUTOTRACING ✓ 持续性能剖析 Profiling ✓ 开源生态融合。
- 低损耗设计（**损耗 1%**）：慢速路径 / 异常上下文 / 精准埋点 / 高效数据传输；eBPF vs 内核模块取舍；内核结构体、K8S POD、集群数据关联打通；具备降级能力防二次故障。
- 观测维度覆盖：TCP/IP 协议栈（丢包/收发延迟/Qdisc 延迟/硬件丢包）、CPU 调度器（争抢/延迟/高频 syscall）、容器 D 状态/异常 fork、VFS/Block IO（IO 调度/设备响应/刷盘阻塞）、内存（kswapd/直接回收）、中断关闭时长（硬/软中断直方图）。

**③ 云上网络可观测性（周昱杉）**：颠覆传统的全景洞察 —— 主题与 HUATUO/perf-prof 同属"全景可观测"主线。
**④ 云原生可观测性增强与隔离（李辉）**：云原生场景下的可观测与隔离实践。

### 二、AI 驱动运维（AIOps）

**⑤ OCAI — 开放智能体驱动的智能运维新范式（李伟杰，腾讯 OpenCloudOS 运维工具 SIG）**
- 问题背景：告警阈值规则滥用→运维淹没在告警中、核心错误被掩盖；故障信息碎片化（日志/链路/依赖需反复比对）；场景复杂（10000+ 软件包、C 库、内核、硬件）；性能/稳定性/成本无完美解，需找最优平衡点。
- 方案：**OCAI-Agent**（一键安装、快速提效）→ Agent 驱动智能运维 → 未来 **全新 OCAI-Agent + MCP 工具规划**（用 MCP 暴露运维工具给智能体）。

**⑥ 多智能体协同（刘泽佑）**：Linux 发行版软件包无人值守维护工作流（AI 多智能体重构维护流程）。
**⑦ AI Agent 实时驱动内核补丁移植自动化（杨达明）**。

### 三、异构算力与国产软硬件生态

**⑧ 沐曦 GPU 训练/推理优化（叶伟，沐曦集成电路）**
- 自研 GPU + **MXMACA®** 软件生态，兼容国际主流 GPU 生态，目标"国产 GPU 零成本迁移"；从芯片→PCIe/OAM 模组→服务器/超节点→集群贯穿。
- 软件栈层次：
  - 驱动层 **KMD / UMD**；用户层 C++（MXMACA C++）、Python、Triton。
  - 异构计算软件栈 **MXMACA**：macaRT、Converter、Quantizer、MCCL（集合通信）、mcDNN（DNN 加速）。
  - 基础软件层：PyTorch / TensorFlow / PaddlePaddle / MindSpore；集群运维 **mx-smi / mx-exporter / mx-report**、Kubernetes、Prometheus/Grafana。
  - 大模型训练&推理加速平台与集群管理工具（mx-dcm）。

**⑨ RISC-V（孙敏）**：RVA23 路线下的 RISC-V 架构创新与生态协同。
**⑩ 达梦数据库（郭一兵）**：达梦与 OpenCloudOS 软件栈联合优化。
**⑪ 宝塔面板（杨金泽）**：轻量云开发者友好的运维实践。
**⑫ 嘉为科技（冯立亮）**：与 OpenCloudOS 生态共建（从适配到护航）。

### 四、横向提炼

- **可观测主线贯穿全场**：perf-prof（工具）、HUATUO（内核全景）、OCAI（智能体运维）、云原生可观测（李辉）——与 [[50-reference/npp-timer-mechanism]]（观测/定时器机制）主题同源，但本大会侧重"操作系统级全景观测 + AI 运维"，而非内核定时器实现。
- **国产替代闭环**：CPU(RISC-V/AMD)、GPU(沐曦 MXMACA)、DB(达梦)、OS(OpenCloudOS) 形成软硬一体国产栈。
- **工程化要点**：低损耗(1%)、零存储开销、自动化追踪(AUTOTRACING)、MCP 工具化是 2025 年可观测/运维工具的共同演进方向。

### 双链

- 内核观测/定时器机制：[[50-reference/npp-timer-mechanism]]
- 动态库/运行时机制：[[50-reference/dlopen-internal-memory]]
