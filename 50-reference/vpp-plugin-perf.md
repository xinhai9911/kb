...
> 前置：[[20-protocols/vpp|VPP 知识]]（向量化原理、缓冲管理）、[[50-reference/vpp-plugin-dev|VPP 插件开发]]（node/plugin 写法）、[[50-reference/vpp-usage|VPP 使用方法]]（CLI 与 `show runtime`/`show cpu`）。
...
- 触发实际工作时用 **interrupt pending** 唤醒一个轻量 INTERNAL cleaner node（见 NPP：[[50-reference/npp-timer-mechanism|PROCESS 定时 + INTERRUPT 触发 cleaner]]），让重活在节点图里被向量化处理、受 `show runtime` 统计。
...
本项目 [[50-reference/npp-timer-mechanism|NPP]] 的流表老化是典型调优对象：
...
- [[20-protocols/vpp|VPP 知识]]、[[50-reference/vpp-plugin-dev|VPP 插件开发]]、[[50-reference/vpp-usage|VPP 使用方法]]
