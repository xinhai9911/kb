...
> 注：开源栈（Yosys + nextpnr + Verilator）适合小规模 + Lattice ECP5 / iCE40 / Gowin，详见 [[entities/fpga-vendors]]。
...
- **时钟规划**：先 BUFG / BUFGCE 做全局时钟，再用 PLL / MMCM 生成所需频率 → [[20-protocols/fpga-ip-catalog|FPGA 常用 IP 核速查]] §时钟管理
- **复位策略**：异步复位 + 同步释放（`reset_sync`），多复位域走 CDC（握手 / 格雷码 / 异步 FIFO）
- **跨时钟域 (CDC)**：握手协议、格雷码、异步 FIFO 三种主路径，详见 [[20-protocols/fpga-design-patterns]]

### 5.2 总线架构选型

| 总线 | 场景 | 典型挂载 |
|------|------|----------|
| **AXI4** | 高带宽（DDR / Ethernet） | Xilinx AXI IP → [[20-protocols/fpga-axi4-bus|AXI4 总线协议深度]] |
...
| 模块 | 基地址 | 大小 | 用途 |
|------|--------|------|------|
| DDR | 0x8000_0000 | 1GB | 外部 DDR → [[20-protocols/fpga-ddr-memory|DDR 存储器接口]] |
| BRAM | 0x4000_0000 | 256KB | CPU 程序 / 数据 |
| UART | 0x4001_0000 | 4KB | 寄存器配置 → [[20-protocols/fpga-uart-spi-i2c-rtl|UART/SPI/I2C]] |
...
- **Vivado IP Catalog**（图形化封装）→ [[20-protocols/fpga-ip-catalog|FPGA 常用 IP 核速查]]
- **HLS**（高级综合，C/C++ → RTL）
- **软核 IP**（MicroBlaze、RISC-V Rocket、Bluespec）→ [[20-protocols/fpga-riscv-softcore|RISC-V 软核在 FPGA 上部署]]
...
> 完整约束参考：[[20-protocols/fpga-constraints-xdc-sdc|FPGA 综合约束 XDC/SDC 写法]]
...
详细工具链 + CI 化见 [[50-reference/fpga-verification]]。
...
6. **可重用 IP**：内部 IP 仓库，分版本管理 → [[20-protocols/fpga-ip-catalog|FPGA 常用 IP 核速查]]
...
| 阶段 | 推荐笔记 |
|------|---------|
| **基础** | [[20-protocols/fpga]]、[[50-reference/fpga-usage]] |
| **进阶** | [[20-protocols/fpga-constraints-xdc-sdc]]、[[20-protocols/fpga-design-patterns]] |
| **实战** | [[20-protocols/fpga-uart-spi-i2c-rtl]]、[[20-protocols/fpga-axi4-bus]] |
| **SoC** | [[20-protocols/fpga-riscv-softcore]]、[[20-protocols/fpga-zynq-soc]]、[[20-protocols/fpga-ddr-memory]] |
| **高阶** | [[50-reference/fpga-verification]]、[[entities/fpga-vendors]]、[[20-protocols/fpga-ip-catalog]] |

## 10. 全部笔记导航

| 分类 | 笔记 | 一句话 |
|------|------|--------|
| **总入口** | [[20-protocols/fpga]] | 架构/流程/HDL/时序/对比 |
| **设计模式** | [[20-protocols/fpga-design-patterns]] | FSM/流水线/FIFO/握手/AXI-S |
| **总线** | [[20-protocols/fpga-axi4-bus]] | AXI4 Full/Lite/Stream 深度 |
| **存储** | [[20-protocols/fpga-ddr-memory]] | DDR3/4/5 + MIG IP |
| **外设** | [[20-protocols/fpga-uart-spi-i2c-rtl]] | 三种接口可综合模板 |
| **SoC** | [[20-protocols/fpga-zynq-soc]] | Zynq PS/PL 协同 |
| **CPU** | [[20-protocols/fpga-riscv-softcore]] | RISC-V 软核部署 |
| **约束** | [[20-protocols/fpga-constraints-xdc-sdc]] | XDC/SDC 速查 |
| **IP 核** | [[20-protocols/fpga-ip-catalog]] | MMCM/FIFO/DMA/XPM |
| **工具链** | [[50-reference/fpga-usage]] | iverilog/Vivado/上板 |
| **验证** | [[50-reference/fpga-verification]] | Testbench/SVA/CI |
| **厂商** | [[entities/fpga-vendors]] | 6大厂商/开源生态 |

## 延伸

- 起点概念：[[20-protocols/fpga]]
- 设计模式：[[20-protocols/fpga-design-patterns]]
- 工具链与上板：[[50-reference/fpga-usage]]
- 验证方法：[[50-reference/fpga-verification]]
- 厂商与开源栈：[[entities/fpga-vendors]]
