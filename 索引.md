...
- [[concepts/LLM 推理 优化|LLM 推理优化]]
- [[concepts/推测解码|推测解码（Speculative Decoding）]]
- [[concepts/分布式推理|分布式推理（TP/PP/EP 多 GPU 推理）]]
- [[concepts/模型 压缩 蒸馏|模型压缩与知识蒸馏]]
- [[concepts/ascend software stack|昇腾 AI 软件栈（CANN / MindSpore / MindIE）]]

### 推理引擎（实体）

- [[entities/vllm|vLLM 推理引擎（PagedAttention）]]
- [[entities/sglang|SGLang 推理引擎（RadixAttention）]]
- [[entities/llama.cpp|llama.cpp 推理引擎（GGUF 量化 / CPU 优先）]]
- [[entities/tensorrt-llm|TensorRT-LLM 推理引擎（NVIDIA 极致优化）]]
...
- [[sources/LLM 推理 优化|LLM 推理优化综述]]
- [[sources/中文 LLM 全景|中国大模型生态分析]]
- [[sources/HuggingFace 生态|Hugging Face 生态系统]]

### 推理引擎深度解析（源码 / 架构）

- [[sources/vLLM-Deep-Dive|vLLM 深度解析]]
- [[sources/vLLM 源码 导读|vLLM 源码导读（本地仓库）]]
- [[sources/vLLM 对外接口与运行参考|vLLM 对外接口与运行参考]]
- [[sources/SGLang-Deep-Dive|SGLang 深度解析]]
- [[sources/SGLang 源码导读|SGLang 源码导读（模块地图 / MLA / Rust / 分布式）]]
- [[sources/llama.cpp-Deep-Dive|llama.cpp 深度解析（GGUF / 量化 / 多后端）]]
- [[sources/推理引擎 性能对照与基准|推理引擎 性能对照与基准（公开量级 + 自测方法论）]]
