...
> 与 [[50-reference/vpp-usage|VPP 使用方法]] 互补：本文讲"是什么 / 为什么"，那边讲"怎么用"。
...
- `VLIB_NODE_TYPE_PROCESS` —— 协作式多任务协程（见下），如 `flowtable-clear-process`（参见 NPP 的 [[50-reference/npp-timer-mechanism|定时触发机制]]）。
...
| NPP | 本项目 [[50-reference/npp-timer-mechanism|NPP]] 即基于 VPP/VLIB 框架扩展插件 |
...
- 使用方法见 [[50-reference/vpp-usage|VPP 使用方法]]。
