...
> 用法/工具链见 [[50-reference/fpga-usage|FPGA 使用方法]]。本文讲"是什么 / 为什么"。
...
详细说明与命令见 [[50-reference/fpga-usage|FPGA 使用方法]]。
...
- **跨时钟域 (CDC, Clock Domain Crossing)**：不同时钟域间传信号必须用**两级同步器（2-FF synchronizer）**、握手或异步 FIFO（见 [[50-reference/fpga-usage|FPGA 使用方法]] §CDC）。
...
- **网络数据面**：FPGA 可做线速包处理、协议识别、加解密卸载（与 [[20-protocols/vpp|VPP]] 用户态方案互补：FPGA 在硬件层、VPP 在软件层）。
- **交换芯片**：盛科/博通交换 ASIC 的前端常配 FPGA 做灵活匹配（参见 [[50-reference/sources/chips/centec-ctc7132|CTC7132]]）。
...
- **可复用 RTL 范式**：FSM、流水线、同步/异步 FIFO、握手、AXI-Stream、仲裁器 → [[20-protocols/fpga-design-patterns|FPGA 常用设计模式]]
- **验证方法论**：自检查 testbench、SVA/PSL 断言、功能覆盖率、约束随机、Verilator CI → [[50-reference/fpga-verification|FPGA 验证方法]]
- **工具链实操**：仿真（iverilog/Verilator/GHDL）、综合实现（Vivado/Quartus）、CDC 写法、上板 → [[50-reference/fpga-usage|FPGA 使用方法]]
- **厂商与开源生态**：AMD/Intel/Lattice/Microchip 选型、Yosys/nextpnr/openFPGALoader → [[entities/fpga-vendors|FPGA 厂商与开源工具链]]
...
- **跨时钟域**是 STA 难点：用 `set_clock_groups -asynchronous` 告诉工具两条时钟无关，避免误报；物理同步用 [[20-protocols/fpga-design-patterns|设计模式]] §异步 FIFO。
...
- [[50-reference/fpga-usage|FPGA 使用方法]]（Vivado/Quartus/Verilator/Icarus 实操、仿真、CDC、上板）
...
- [[entities/fpga-vendors|FPGA 厂商与开源工具链]]（选型 + Yosys/nextpnr 生态）
