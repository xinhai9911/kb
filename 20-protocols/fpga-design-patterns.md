...
created: 2026-07-29
summary: >-
    FPGA 可复用的 RTL 设计模式：有限状态机(FSM)、流水线(Pipeline)、同步 FIFO 与异步 CDC FIFO、握手协议、AXI-Stream 数据流、仲裁器(Arbiter)。每个模式给出可综合写法、适用场景与坑点。配合 [[20-protocols/fpga|FPGA 知识]] 与 [[50-reference/fpga-usage|FPGA 使用方法]]。
category: reference
updated: 2026-07-29
sources: []
...
> 配合 [[20-protocols/fpga|FPGA 知识]]（架构/时序）、[[50-reference/fpga-usage|FPGA 使用方法]]（工具链/仿真）。本文给可直接套用的 RTL 范式。
...
**不要**手搓除非必要——CDC FIFO 的空满判定极易出错，优先用原厂 IP。详见 [[50-reference/fpga-usage|FPGA 使用方法]] §CDC。
...
- 厂商/工具：[[entities/fpga-vendors|FPGA 厂商与开源工具链]]
