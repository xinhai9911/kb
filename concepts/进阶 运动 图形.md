---
aliases: ["advanced-motion-graphics"]
...
[[concepts/视觉特效 运动 图形]] 覆盖了运动跟踪、画面稳定、混合模式等基础 VFX 概念。AE 动态图形是在这些基础上叠加的视觉增强层——文字动画、UI 动效、Logo 演绎、数据可视化、标题序列等。

[[concepts/字幕 本地化]] 中的 Lower Third 标题设计通常直接在 AE 中完成。
