...
视觉特效（VFX）和动态图形（Motion Graphics）是后期制作中画面流水线的两大核心扩展。[[concepts/色度 键 合成|绿幕抠像]]覆盖了合成的一个子集，本篇扩展到运动跟踪、画面稳定、混合模式理论、After Effects 工作流和标题动画。
...
- [[concepts/色度 键 合成]] — 绿幕/蓝幕抠像（VFX 合成的基础）
- [[entities/Adobe Premiere Pro]] — 与 AE 动态链接的 NLE
- [[concepts/剪辑 转场]] — 转场与动画的中间地带
